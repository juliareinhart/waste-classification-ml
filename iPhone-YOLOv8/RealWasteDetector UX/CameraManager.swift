...
final class CameraManager: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    // Added for stats
    var onDetection: ((String, Double) -> Void)?
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        /* To add the new model to Xcode
         1. Drag your new .mlmodel or .mlpackage into Xcode.
         2. Check Copy items if needed.
         3. Make sure your app target is checked.
         4. Delete or keep the old model, but note the new generated model class name.
          Find the two arrows <== below */
        isProcessingFrame = true
        defer { isProcessingFrame = false }

        do {
            let config = MLModelConfiguration()
            let model = try best(configuration: config)//<== Model name to replace
            let output = try model.prediction(image: resizedPixelBuffer)
            let candidates = findCandidates(in: output.var_910, threshold: 0.4)//<== check and replace the output name
            ...
            DispatchQueue.main.async {
                ...
                // Add the stats
                self.onDetection?(label, Double(result.score))
            }
    ...
    func detectPhoto(image: UIImage) -> [PhotoDetectionResult] {
        guard let resizedPixelBuffer = image.toPixelBuffer(width: 640, height: 640) else {
            return []
        }
        ...
        do {
            ...
            let scaledWidth = rawWidth * tuning
            let scaledHeight = rawHeight * tuning
            ...
            return [
                PhotoDetectionResult(
                    ...
                    rect: CGRect(
                        x: CGFloat(result.x) - scaledWidth / 2,
                        y: CGFloat(result.y) - scaledHeight / 2,
                        width: scaledWidth,
                        height: scaledHeight
                    )
                )
            ...
            print("Photo detection failed: \(error)")
            return []
        }
    }
...
extension UIImage {
    func toPixelBuffer(width: Int, height: Int) -> CVPixelBuffer? {
        ...
        guard let context, let cgImage = self.cgImage else {
